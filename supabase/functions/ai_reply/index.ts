   ...
   serve(async (req) => {
     try {
       ...
       console.log("ai_reply: call OpenAI");
         ...
         messages: [
           {
             role: "system",
             content:
               "あなたは感謝日記を書くユーザーをサポートするコーチです。丁寧語で 100 字以内でフィードバックしてください。",
           },
       ...
       const replyText = completion.choices[0].message.content?.trim() ??
         "素敵ですね！";
       console.log("ai_reply: OpenAI ok →", replyText);

       /* ❹ ElevenLabs TTS (任意) ----------------------------- */
       let audioPath: string | null = null;
       if (elevenKey) {
         console.log("ai_reply: call ElevenLabs");
         ...
         if (!tts.ok) {
           console.error("ai_reply: ElevenLabs error", await tts.text());
         } else {
           const buf = new Uint8Array(await tts.arrayBuffer());
           audioPath = await uploadAudio(buf, record.diary_id);
           console.log("ai_reply: audio uploaded →", audioPath);
